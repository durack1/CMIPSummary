...
\Author[2]{Michael}{Lautenschlager} % no ORCID, no reply, move to acknowledgements
...
\begin{itemize}
  ...
  \item Realized potential of distributed Community MIPs - with CMIP and umbrella project for many different climate research threads
...
To provide context for the most recent phase \citep[CMIP6;][]{eyring_overview_2016} and the planning of the project’s future, we discuss the efforts dating back to the MIP origins and their contribution to making climate community collaboration what it is today. Small teams or individuals initiated many of the steps taken. While evidence of this history is scattered in literature, we attempt to bring them together to tell a coherent story of how CMIP got to where it is today. Our emphasis details the pragmatics of what it took to progressively scale efforts up, most recently delivering CMIP6, thus this review describes the evolution of how the MIPs were implemented and the coordinated efforts to establish a minimal infrastructure to make that possible. In \autoref{sec:cmip6InContext}, we discuss MIP phases from 1990 to the present. In \autoref{sec:cmip6ProjectDesign}, we highlight the CMIP6 experimental design, and in \autoref{sec:CMIPSupportingOrgsAndInfra}, the supporting organizations and infrastructure, \autoref{sec:CMIP6SupportingProjects} describes CMIP6 supporting projects, \autoref{sec:CMIP6Impact} is an attempt of quantifying the project's impact, \autoref{sec:CMIP6Completion} CMIP6 completion and next steps. In \autoref{sec:Conclusions}, we look forward to the upcoming CMIP7 phase and note some recommendations to guide ongoing MIP activities.
...
\cred{\textbf{Attn: Jonathan G., Bryan L; UK/EU parallel activities?}}
...
\mycomment{Jerry P: I really don’t have anything to add other that The FANGIO project was specifically designed to look at feedback mechanisms. We invited the participants to run exactly the same experiment using a prescribed sea surface perturbation so by prescribing the climate change we sorted out various feedback mechanisms. We concluded that cloud feedback among different models was the major cause of the differences in model sensitivity.}
...
\cred{\textbf{Attn: Jerry M. representative of reality?}}

\mycomment{
%%##### COMMENTS %#####
Ron S: This text seems right to me. CMIP4 was labeled as such in hindsight
%#####
}
...
By March 2020, the CMIP6 project included 322 experiments, contributed across 23 MIPs (\autoref{tab:tab2-CMIP6MIPs}). Compared to the prior phases, this order of magnitude growth marked enhanced participation and complexity (\autoref{fig:fig1-MIPGrowth}), capturing an expansion of climate science and the successful federation of the project structure and management.
...
Before CMIP5, PCMDI and its collaborators managed all the data distribution responsibilities for AMIP and CMIP. 
Early in the planning of CMIP5 it became clear that PCMDI could not manage the expected number of data producers and consumers. PCMDI requested help from the Global Organisation of Earth System Science Portals (GO-ESSP), to help design and deliver a globally distributed data infrastructure. The resulting infrastructure, became the Earth System Grid Federation (ESGF). (The name being an evolution, like the software, of the pre-existing D.O.E initiatve "the Earth System Grid".)

With scale, and the advent of the ESGF, the entire CMIP process began to depend more and more on the underpinning technical infrastructure (which, as will be seen below, went beyond just ESGF alone). To coordinate the development and delivery of that infrastructure, the WGCM established the WGCM Infrastructure Panel (WIP) in 2014, to work in parallel and in collaboration with the CMIP Panel to facilitate the production and consumption of CMIP data products. The WIP maintained (and still maintains) the necessary standards and policies for model data sharing and ensured that the various infrastructure components could be integrated and function smoothly. Its work was critical to the success of CMIP6.

For CMIP5, and since, many independently-funded efforts contributed to the supporting infrastructure. Amongst them, key roles were played by the Climate and Forecast (CF) Metadata Conventions (\autoref{sec:CFConventions}), used to standardize the content of model output files, and the CMIP Controlled Vocabularies (CVs; \autoref{sec:CMIPCVs}), used to 
 provide a limited set of terms used to describe and uniquely identify datasets and to aid in searching for data of interest.
 The use of these standards is now integral to the intercomparison of model data. 
 PCMDI developed software (CMOR) to impose the use of these conventions and vocabularies, and the process of putting data into the right format with the right information content became known as "CMORization" whether or not CMOR was used. 

Data conforming to the necessary standards can be published into the ESGF, where it is findable and accessible worldwide. 
A subset of the CMIP archive is also replicated into the  IPCC Data Distribution Centre (IPCC DDC; \autoref{sec:IPCC-DDC}) where, augmented with extra metadata, it is expected to be preserved and available indefinitely. 
...
\cred{\textbf{Attn: David H., Jonathan G., any edits to suggest?}}
...
Subsequently, the CF conventions were adopted by CMIP, starting in phase 3. Since that time, CF gained in popularity across climate sciences and beyond. A formal governance structure was established (Lawrence et al 2006), and an ever-expanding group of volunteers now maintains these conventions (see \url{https://cfconventions.org}). Evolution (while maintaining backward compatibility) has been ongoing through several releases, the latest being version 1.12. The stability of CF, the vibrant community supporting it, and the development of software that can interpret CF data have been foundational to the widespread use of CMIP data. 
...
\cred{\textbf{Attn: UK/EU Bryan L., Phil K., Stephan K.; GO-ESSP?; Guillaume L., Atef B-N.: We need a paragraph about Synda/and replication in this subsection}}
...
\cred{It should be added somewhere that the IPCC DDC relies on CMIP standards (like many other downstream services), even more with the introduction of the IPCC FAIR Guidelines into the AR6.}
\cred{You cannot write "DKRZ IPCC DDC". There is only one IPCC DDC, jointly managed by several partners. DKRZ is a DDC Partner.}
\cred{
...
\cblue{\textbf{Live evolving section...}}
...
During the planning for CMIP5, the WGCM endorsed an attempt to formally collect model documentation under the auspices of the European ``Metafor'' project, which was initiated in 2008 \citep{guilyardi_cmip5_2011,lawrence_describing_2012}. Building on work for the US ``Curator'' project, which began in 2005, a complex metadata system was designed and built to capture provenance activities in the entire process, from experiment design through model description and execution to data delivery. Unfortunately, the system was delivered too late, with too little user support, to be used for CMIP5. However, the information gained was used in developing the model description tables for the IPCC AR5. A similar attempt was undertaken for CMIP6, where many of the technical lessons from the CMIP5 activity were addressed \citep{pascoe_documenting_2020}. Unfortunately, few modelling groups provided the requested information, and the infrastructure to deliver the collected information has been allowed to wither. As a consequence, important initiatives to understand the impact of model structure on climate variability have had to rely on a painstaking and incomplete analysis of the available public information \cite[e.g.][]{boe_interdependency_2018} or post-hoc analysis of results \citep[e.g.,][]{masson_climate_2011,knutti_climate_2013}.

The importance of such documentation is still recognised, and so for CMIP7, a simpler process which can be deployed early, easily maintained alongside existing CVs, and which should be easier to populate, has been designed. This ``Essential Model Documentation'' will be required before data can be published into the CMIP7 component of the ESGF.
...
\cred{Ron S: The notable MOM ocean model documentation needs to be noted here. It was the first complete model documentation I know, and it is still the best I know.}
...
With the support of IS-ENES (see \autoref{sec:IS-ENES}) in collaboration with DoE, ESGF contributors from the Italian CMCC Foundation (Euro-Mediterranean Center on Climate Change), developed the metrics and dashboard capability. By 2015, the first version of the ESGF dashboard was live providing project statistics for CMIP5, in addition to the smaller supporting projects obs4MIPs (see \autoref{sec:CMIP6SupportingProjects-obs4MIPs}) and CORDEX-CMIP5 \citep{williams_5th_2016}. This service captures a comprehensive set of data usage and data archive metrics both for a single ESGF node, and at the cumulative federation level. The data usage information provides the number of successful downloads, along with the number of distinct downloaded files, with granularity that enables an interrogation of these metrics by project, variable, institution, model, and experiment. In addition, data archive information is available per project, quantifying the total number of published datasets, data volume and granular information around the CMIP project contributing models and institutions.

Today, the CMCC ESGF Dashboard continues to provide metrics across the ESGF projects: CMIP6, CMIP5, CORDEX-CMIP5, input4MIPs (see \autoref{sec:CMIP6SupportingProjects-input4MIPs}) and obs4MIPs \citep{fiore_climate_2019}. The service is collating project data and download statistics from a subset of the total contributing nodes across projects (see \autoref{tab:tab1-MIPsThroughTime}). For the CMIP5 project, 7 ESGF nodes are currently reporting data and download statistics, and for CMIP6 9 nodes are reporting, with temporal coverage from 2018 through to the present. This is a subset of the total 30 nodes that are currently publishing data to the archive.

The CMCC-provided dashboard statistics, in addition to archived ESG II statistics from the single LLNL/PCMDI CMIP3 project portal (2004-2009) have been used in the preparation of \autoref{fig:fig5-MIPDownloads}.
...
\cblue{\textbf{Karl this would benefit from you taking a pass}}

\cred{
%%##### COMMENTS %#####
Ron S: This greatly undersells CMIP. The IPCC as we know it would not exist without CMIP. This paper can claim a lot of IPCC’s impact on society. Organizing climate scientists, data scientists, networking scientists (and so on) is another big factor leading to CMIP’s success. Getting funder buy-in is also a huge factor.
CMIP undersold - One idea is to count the number of figures in the various IPCC SPMs which contain CMIP data. I would guess that it is about half of them. A harder and more time consuming task is to count references in the IPCC which use CMIP data....would not recommend this path. :)
I think a summary sentence like - "The IPCC has achieved remarkable success; CMIP has allowed much of that success to occur." I think this could get by the reviewers. In a sense, the whole paper supports this statement. Another point is that CMIP would be a success without the IPCC, you can't say the reverse.
Then go into what allowed CMIP to be successful - science insights, common data formats, common variable naming, detailed internet protocols, etc....and above all lots of volunteers. People with a vision.
%#####
}


By establishing benchmark experiments along with innovative targeted experimentation, and by facilitating access to the results, CMIP and the other MIPs its infrastructure has supported have fundamentally changed the expectations of and research practices in climate modeling. It has become common practice to assess areas where models should not be trusted by considering where their results diverge. Agreement among models does not, of course, imply correctness, but the multi-model perspective enables a richer interpretation than results from a single model.  
...
This article is dedicated to W. Lawrence "Larry" Gates, the first PCMDI Director, whose vision, leadership, and skill in building community engagement and consensus, garnered acceptance of the open and systematic climate model analysis and led to the transformative impact of CMIP.
...
We gratefully acknowledge the assistance of Vaishali Naik (NOAA Geophysical Fluid Dynamics Laboratory, Princeton, USA) for assistance better understanding the climate forcing experience of modelling groups through the CMIP5 phase.
