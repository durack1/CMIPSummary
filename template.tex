...
Subsequently, the CF conventions were adopted by CMIP, starting in phase 3. Since that time, CF gained in popularity across climate sciences and beyond. A formal governance structure was established, and an ever-expanding group of volunteers made contributions extending and correcting flaws in the original conventions \cred{(REFERENCE WEBSITE HERE: https://cfconventions.org/)}. The evolution (while essentially maintaining backward compatibility) has been ongoing through several releases, the latest being version 1.12. The stability of CF, the vibrant community supporting it, and the development of software that can interpret CF data have been foundational to the widespread use of CMIP data. 
...
\label{sec:CMIPCVs}
\cred{Attn: Paul D.; MIP era realm evolution, dovetailing Variable request section}
...
For a project like CMIP, which has evolved markedly over its four decades of operation, it is challenging to accurately capture its impact and apportion recognition to its contributors and participants, and identify the breadth and diversity of users of generated data downstream. Moving to an open data paradigm (and equivalently open data licenses, \autoref{tab:tab1-MIPsThroughTime}) with CMIP3, further complicated project attribution, leading to mirror sites creating secondary data repositories outside the ESGF licensing and access control \citep{balaji_requirements_2018}. In addition, compute services co-located with the primary ESGF replication nodes (e.g., PCMDI LLNL, DKRZ, and CEDA), other regional nodes (e.g., NCAR, NCI) and the replication of datasets into the commercial cloud (e.g., PANGEO began to serve larger and larger regional communities, further complicating data access and use assessment.

Downloads, citations.. imperfect measures of impact.
