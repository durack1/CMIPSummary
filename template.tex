...
\title{The Coupled Model Intercomparison Project phase 6 (CMIP6): A review of project evolution and future}
\mycomment{Pete G alternate: The Coupled Model Intercomparison Project phase 6 (CMIP6) made possible by an evolution of project implementation}

% \Author[affil]{given_name}{surname} % Author {first initial.}{last}
\Author[1]{Paul J.}{Durack}
\Author[1]{Karl E.}{Taylor}
\Author[1]{Peter J.}{Gleckler}
\Author[2]{Matthew}{Mizielinski}
\Author[3]{Daniel}{Ellis}
\Author[1]{Jiwoo}{Lee}
...
\textbf{Dean W.} (ESGF), \textbf{Bryan L.} (ESGF).
...
A critical component MIP experimentation is specifying what simulation output to save. It is a trade off between the efforts required by modelling groups to fulfill a request, available storage, and existing and potential future science needs. Despite advancing technology, saving everything (i.e., all prognostic variables at every timestep) has never been realistic. In AMIP1/2, CMIP1/2/2+/3/5, consideration was made in consultation with MIP chairs and modelling groups about what questions or analyses will be pursued with a given simulation that dictates how much data is saved. As MIPs emerged, more scientists began analyzing simulations, and it was not always clear a priori what would be studied. The most basic or routine climate model output is arguably monthly means, so one question often is which variables to save. However, even in the early days of AMIP1, scientists were interested in process-level analyses to understand model responses more deeply, which required daily or even higher temporal frequency fields.

In AMIP1, boundaries were pushed based on the amount of saved data. To define the standard output list, discussions between the WCRP WGNE panel, PCMDI, contributing modellers, and other scientists who were closely engaged in the process that defined the request. Enough temporal mean data was saved to enable many scientists to complete their diagnostic subprojects \citep{gates_amip_1995} evaluating a diversity of model characteristics in ways that had never been done before. But it was not enough, and only the beginning. For AMIP2, continued discussions focused on more in-depth scientific investigations and advanced a more comprehensive set of AMIP standard output. For the first time, this included 6-hourly data, in addition to the monthly mean data requested in AMIP1 (\autoref{tab:tab1-MIPsThroughTime}), expanding the variables from 32 to 114 (see \autoref{tab:tab1-MIPsThroughTime}). In addition, monthly means of selected co-variances that needed to be computed on the fly were added, offering the possibility of evaluating the Lorenz energy cycle as simulated by the models. Co-variances facilitated some interesting work \citep{boer_energy_2008}. Still, it was also a lesson in the delicate balance of what output to save because they are computationally expensive products for modelers to provide. Since AMIP2, co-variances have not been part of the routine output request and have only been saved for targeted studies.

The CMIP1 standard output began very conservatively when contrasted to AMIP2, with just 23 variables requested (\autoref{tab:tab1-MIPsThroughTime}) representing a handful of time averages and three monthly mean time series. Like AMIP, what to save was defined by scientists actively involved in advancing the project, including the WGCM and PCMDI. Like AMIP1, the initial data was only a stepping stone of what was to come.  For CMIP3, there was a somewhat more concerted effort to define the model output, but it was still done within a small community that was open to feedback from interested scientists but had to balance the practical limits of the data volume with the potential benefits. <Should the infamous Taylor spreadsheets be pointed to?>.
...
\cred{Attention Bryan Lawrence, David Hassell}
\citet{guilyardi_cmip5_2011}
\citet{pascoe_documenting_2020}
...
\cred{Attention Jiwoo L; Point to Hassler et al., 2024 Model Benchmarking TT review paper; PMP papers; ESMValTool papers; other papers?}
...
\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{240918_durack1-cmip6-experiments_240903.png}
