...
\Author[30]{others please}{add yourselves - will alphabetically reorder once final}
% \affil % Affiliations
...
\textbf{\sout{Jiwoo Lee, PCMDI}} (\autoref{sec:CMIP6SupportingProjects-CoordEval}: Coordinated evaluation);
\textbf{\underline{Martina Stockhause, DKRZ}} (\autoref{sec:IPCC-DDC}: IPCC-DDC; \autoref{sec:DataCitation}: Data citation);
\textbf{\underline{Bryan Lawrence, David Hassell, UReading}}; Eric Guilyardi, IPSL (\autoref{sec:ModelDocumentation}: Model (and data) documentation);
\textbf{\underline{Guillaume Levavasseur, Atef Ben-Nasser, IPSL}} (\autoref{sec:CMIPErrata}: CMIP errata);
\textbf{\underline{Sandro Fiore, CMCC/UTrento}} (\autoref{sec:CMIPDataDownloads}: Data downloads);
\textbf{\underline{Helene Hewitt, John Dunne, CMIP Panel}}: CMIP7 handoff;
\textbf{Vaishali Naik}: (\autoref{sec:CMIP6SupportingProjects-input4MIPs} forcing);
\textbf{Sebastien Denvil, Ruth Petrie, CDNOT}: \autoref{tab:tab1-MIPsThroughTime}/ESGF
}

\cred{
Reviewers to engage
\textbf{\sout{Curt Covey}, Gerry Potter, Dave Bader} (DOE/PCMDI);
\textbf{Dean Williams, Bryan Lawrence, ESGF-XC membership?} (\autoref{sec:CMIPSupportingOrgsAndInfra}: ESGF);
\textbf{Vaishali Naik, Jean-Francois Lamarque, Veronika Eyring}; (\autoref{sec:CMIP6SupportingProjects-input4MIPs}: forcing data CMIP5 through CMIP6;
\textbf{\sout{Curt Covey}, Jerry Meehl, Ron Stouffer, Broader panel past} (CMIP Panel);
\textbf{Bryan Lawrence, Balaji V., ?; Broader panel past and present} (WIP Panel);
}
...
\begin{figure}
    ...
    \caption{Experiments, models, institutions, and countries contributing to completed Model Intercomparison Project (MIP) phases, from 1989 to today. The project's growth over the most recent phase reflects a growing community appetite for the latest climate data to inform decision-making, a distant evolution from the climate science origin of AMIP1. See \autoref{tab:tabAppA1-MIPExperiments} for an expansion of the experiment lists. \cred{Update before submission - sync with \autoref{tab:tab1-MIPsThroughTime}, \autoref{tab:tabAppA1-MIPExperiments}}}
...
\textbf{Data formats} & Fortran formatted binary & Fortran formatted binary, GRIB {$\rightarrow$} PDRS & GRIB {$\rightarrow$} PDRS & GRIB {$\rightarrow$} PDRS & CF netCDF-3 & CF netCDF-4 "classic" & CF netCDF-4 & CF netCDF-4\\ \hline
...
Prior to CMIP5, PCMDI and its collaborators had alone been responsible for managing all the data distribution responsibilities for AMIP and CMIP.  With CMIP5's growth, it became clear that a more distributed support of the infrastructure was needed, and new partnerships were established.  For the purpose of coordinating the technical aspects of CMIP, the WGCM established the WGCM Infrastructure Panel (WIP), which works in parallel and in collaboration with the CMIP Panel to ensure that CMIP data is made widely available.  The WIP is responsible for maintaining standards and policies for model data sharing and for ensuring that the various components of the infrastructure can be integrated and function smoothly together, and its work was critical to the success of CMIP6. 
...
\cred{Attn: US Dean W., UK/EU Bryan L.;  need to also mention Synda and replication here}
...
By the early 1990s, interest grew in sharing the output of climate models with researchers outside the orginating groups. This led to greater attention paid to clearly defining the model output quantities, ensuring that datasets were self-describing to avoid misinterpretation, standardizing their digital formats to ease data transfers and use, and documenting the model configurations that produced the data. In parallel, there was computer science development focused on generating a digital format that met the needs of growing meteorological data streams, with an aspiration to develop a portable and self-describing file format (where data and metadata that described it were co-contained in files). These developments had led to numerous early releases of the network Common Data Format (netCDF) through the late 1980s and early 1990s. In May 1997, netCDF 3.3 was released, which included a new type-safe interface for the C and Fortran languages. In March 2001, netCDF 3.5.0 was released, which integrated a new Fortran90 interface, making this library suitable for integration with climate model libraries primarily written in Fortran \cred{(Refs?)}.
\mycomment{
...
By 1997, the large and growing MIP archives had accumulated tens of thousands of output files from dozens of model configurations. The diverse and growing international community interested in analyzing and applying results from the models was hampered by the diversity of data formats, structures, and nomenclatures that had been adopted by climate modeling centers.  There would be obvious benefits in standardizing the data. The Gregory, Drach, and Tett \citep[GDT v0;][]{gregory_gdt_1999} standards were first introduced in June 1997 to fill the void. The GDT standard expanded markedly on COARDS, with a prime focus on better enabling GCM output to be self-describing and conform to standard conventions that detailed longitude, latitude, height/depth, coordinate/grid bounds, time, calendars and climatologies, missing values, and data compression. In 1999, the continued development of GDT ceased at v1.4, but the interest it had attracted led to a collaboration of a somewhat larger group of authors who began work developing what became known as the NetCDF Climate and Forecast (CF) Metadata Conventions. Building on and going far beyond COARDS, GDT, and similar efforts, version 1.0 of the CF Conventions was released in October 2003 \citep{eaton_netcdf_2024}.  

Subsequently, the CF conventions were adopted by CMIP, starting in phase 3.  Since that time, CF gained in popularity across climate sciences and beyond.  A formal governance structure was established, and an ever expanding group of volunteers made contributions extending and correcting flaws in the original conventions \cred{(REFERENCE HERE WEBSITE: https://cfconventions.org/)}.  The evolution (while largely maintaining backward compatibility) has been ongoing through a number of releases, the latest being version 1.12.  The stability of CF and the vibrant community supporting it and developing software that can interpret CF data have been foundational to the widespread use of CMIP data. 
...
As coordinated international climate science activities grew from AMIP1 and 2 through the early CMIP phases, it was recognized that imposing increasingly detailed requirements on the data reported would facilitate its use and increase its value.  Early on, it was decided that rather than store multiple variables in a single file, the model output would be organized with each file containing a single variable.  This was done to reduce data download volumes since most analysts of the CMIP data were expected to consider only a small number of variables. 

Beginning with CMIP3, each requested variable was assigned a unique name (e.g., “ts” for surface temperature and “pr” for precipitation rate), and all data were written compliant with the CF conventions.  Additionally, the names of the coordinates and their ordering were standardized, and certain CF optional attributes were made mandatory (e.g., ``cell\_methods'').  Standardization simplified analysis codes that could ingest data produced by multiple models.  In addition, certain metadata were required to be recorded in each file identifying the source of the data (institution, model name, version) and experiment conditions (e.g., experiment name).  Thus, the detailed specifications for CMIP3 (see 
https://pcmdi.github.io/ipcc/IPCC\_output\_requirements.htm) imposed uniformity on the format and structure of the data and metadata that facilitated its use. 

The first collection of controlled vocabularies (CVs) was also introduced in CMIP3.  A CV, in its simplest form, lists the terms used to describe a certain attribute of a dataset.  In CMIP3, there were CVs for variable names (e.g., “tas”, “pr”, “ua”), experiments (e.g., “PIcntrl”, “SRESB1”, “AMIP”), institutions and models (e.g., “BCC-CM1”, “GFDL-CM2.1”, “UKMO-HadGEM1”), and table names (e.g., O1, A1, A2, A3).  The table names identified collections of variables that were all reported at a common “frequency” (1=monthly, 2=daily, 3=3-hourly) and were all associated with either the atmosphere or ocean.  The CVs were used in constructing the directory structure for the data archive with output from different experiments, models, and variables placed in separate aub-directories.  The structure ensured that although the loosely defined template for naming files would not ensure name uniqueness across the entire CMIP3 archive, it would ensure that the names were unique within each lowest-level sub-directory.  

The increased uniformity of the CMIP3 data requirements facilitated subsequent research but imposed a burden on modeling groups who were required to rewrite their data in a new way in what to some was an unfamiliar format.  To assist modeling groups in complying with the model output requirements, the the CMOR1 software \citep{taylor_cmor_2006} along with the CMIP3-CMOR-Tables \citep{doutriaux_cmip3_2005} were developed and made available.  In the tables, the 141 requested variables (see \autoref{tab:tab1-MIPsThroughTime}) were grouped such that variables likely to be processed under a common procedure appeared together.  The detailed data specifications and the clearly defined variables were documented in the CMIP3-CMOR-Tables and in an “output requirements” document \citep{taylor_pcmdi_2005}.  Once the modeling groups prepared their output according to the specifications, it was collected and organized on the PCMDI computing system in an easily searchable directory.  The data archive could be searched and downloaded either directly via anonymous ftp or through the ESG web-based data service.

In CMIP5, the trend toward further standardization continued as the project expanded to include additional specialized MIPs, 26 modelling groups, and 37 experiments (\autoref{tab:tab1-MIPsThroughTime}, \autoref{fig:fig1-MIPGrowth}).  The original CMOR FORTRAN software was revised and recast as C code with FORTRAN and python interfaces \citep{doutriaux_cmor_2011}. New CMIP5-CMOR-Tables \citep{doutriaux_cmip5_2013} were defined, to accommodate the dramatic increase in the variables requested, which now totaled 970  (see \autoref{tab:tab1-MIPsThroughTime}). The data requirements were largely inherited from CMIP3 but were augmented with additional required attributes, some with values drawn from CVs (e.g., “frequency” and “modeling\_realm”).   

A major advance in enabling communication among the various components of software infrastructure supporting CMIP5 was development of a “Data Reference Syntax” or “DRS” \citep{taylor_pcmdi_2012}.   The DRS relied on several descriptors and associated CVs that together uniquely defined each of the datasets contributed to the CMIP5 archive.  The DRS elements were used, for example, in defining the directory structure for the CMIP5 archive and to construct filenames that were guaranteed to be unique within the archive.  The DRS was also essential for communication and data sharing across the data nodes comprising a now federated ESGF data distribution infrastructure.  The DRS descriptors were supplemented with dataset version information assigned during the data preparation and publication processes, which greatly aided the replication of datasets across major ESGF federated data nodes.

For CMIP6 the evolution of the data standards, CVs, and supporting infrastructure continued as the project ballooned in size to include 23 specialized MIPs, 49 contributing modeling groups, and 322 experiments (\autoref{tab:tab1-MIPsThroughTime}). Due to the broadening of model diversity and configurations and the concomitant increase in variables of interest, a new approach to defining the data request was tried whereby the needed variables were placed in a database which could be accessed via an API or displayed in different human-readable forms (see \autoref{sec:CMIP6DR}). In total, 2062 variables were requested across all experiments, and these were all transferred into the CMIP6-CMOR-Tables \citep{nadeau_cmip6_2017} and were relied on by a revised CMOR3.

As new services that enabled formal data citation (see section 4.7) and provided centralized access to extensive model documentation (see section 4.6 began supplementing CMIP's basic service of providing access to CMIP data through ESGF and the IPCC-DDC (see sections 4.1 and 4.2), the CVs became more important for maintaining consistency across the components of the infrastructure.  The earlier practice of publishing simple human-readable CV lists needed to define the CMIP descriptors  led different parts of the infrastructure to replicate the CV information in different specialized formats suitable for a specific application. In some cases the information needed to populate a CV was gathered independently from the other components (e.g., the acronyms used to identify each of the CMIP experiments).  These practices led to some inconsistency in CV content that hampered transparency and communication. Consequently, the WIP decided that for CMIP6   the   fractionation   was increased reliance  assignment of dataset as model and experiment documentation the CMIP infrastructure became 
\cred{STILL MORE TO ADD HERE.}   

\mycomment{
(see \autoref{tab:tabAppD1-CMIP6PrepTools})
...
AMIP2 incorporated additional greenhouse gases, ozone, aerosols, and more complex interactive land components \citep[e.g.,][]{gleckler_amip_1996-1,liang_pcmdi_1997}, and in CMIP1 and CMIP2/CMIP2+, an interactive ocean and sea-ice removed the need for SST and sea-ice boundary forcing.

With the definition of the CMIP3 20C3M/historical experiment \citep[$\sim$1850-2000;][]{meehl_wcrp_2007}, considerable augmentation occurred, with the first historical period datasets hosted by PCMDI alongside existing early AMIP and CMIP data. These data were mostly focused on the SRES future scenarios \citep[2001-2100/2200/2300;][]{nakicenovic_summary_2000}, but did include a small number of historical period forcings including those that had already been collated for the AMIP experiment period (1979-2001; \autoref{tab:tab1-MIPsThroughTime}), with additional needs met by each contributing modelling group independently.

For CMIP5, the increase in model complexity necessitated a far more comprehensive approach to meet the historical experiment forcing needs (\citep[1850-2005;][]{taylor_overview_2012}, and considerable coordination across modelling groups occurred. For anthropogenic emissions of short-lived climate forcers (SLCFs 1850-2000; e.g., CH$_{4}$, CO, NO$_{x}$, etc) dataset creation was led by NCAR, with contributions from a broad community including observationalists and climate modellers \citep{lamarque_historical_2010}. A similar community approach was taken with well-mixed greenhouse gases (WMGHGs; 1765-2300), however the dataset development dovetailed with the generation of the RCP future scenario data extending out to 2300 \citep{meinshausen_rcp_2011}. The development of other datasets that contributed to the project fell back to existing researchers who had been developing datasets focused on solar variability \citep{frohlich_solar_2004}, volcanic stratospheric aerosol emissions \citep{stenchikov_arctic_2006} and land-use changes \citep{hurtt_underpinnings_2006}, however the use of these datasets specifically differed across modelling groups as no centralised dataset coordination was implemented.

For CMIP6, there was a recognition that coordination of forcings was needed. The input datasets for model intercomparison projects (input4MIPs) ESGF project was established, and the collation of forcing data required to undertake the CMIP6 DECK experiments, in addition to other data supporting 14 other Community MIPs were published and made available to participating groups \citep{durack_toward_2018}.

SLCF \citep{hoesly_historical_2018,feng_generation_2020}
Biomass burning \citep{van_marle_historic_2017}
Land use Hurtt et al., 2020; Chini et al., 2021
GHGs \citep{meinshausen_historical_2017}
Volcanic SAOD \citep[e.g.,][]{aubry_new_2020,aubry_independent_2021,aubry_new_2023}
Ozone \citep{checa-garcia_historical_2018}
Nitrogen deposition \citep{hegglin_ccmi_2016} (ESGF)
Solar \citep{matthes_solar_2017}
AMIP \citep{durack_pcmdi_2018}
Aerosol optical \citep{stevens_macv2-sp_2017} \citep[e.g.,][]{fiedler_anthropogenic_2019}
...
\cblue{COMPLETE OUTLINE TODAY - Wed 13 Nov}
...
\autoref{fig:fig3-MIPPhaseCitations}, \autoref{fig:fig4-MIPCitations}, \autoref{fig:fig5-MIPDownloads}, \autoref{fig:fig6-MIPImpact}
...
\codedataavailability{GitHub repositories for data and figures} %% use this section when having data sets and software code available
...
\begin{table*}[htp]
	...
	\resizebox{\textwidth}{!} {
		\begin{tabularx}{0.9\textwidth} {
			...
			\hline
			$\sim$ & CFMIP1 & & 1.0 & $\sim$ & \url{https://github.com/PCMDI/cfmip1-cmor-tables}\\ \hline
			$\sim$ & C-LAMP1 & & 1.0 & $\sim$ & \url{https://github.com/PCMDI/c-lamp1-cmor-tables}\\ \hline
			$\sim$ & IAEMIP1 & & 1.0 & $\sim$ & \url{https://github.com/PCMDI/iaemip1-cmor-tables}\\ \hline
            $\sim$ & CORDEX (CMIP5) & & 2.0 & $\sim$ & \url{https://github.com/PCMDI/cordex-cmor-tables}\\ \hline
			$\sim$ & GEOMIP & & 2.0 & $\sim$ & \url{https://github.com/PCMDI/geomip-cmor-tables}\\ \hline
			$\sim$ & LUCID & & 2.0 & $\sim$ & \url{https://github.com/PCMDI/lucid-cmor-tables}\\ \hline
			$\sim$ & PMIP3 & & 2.0 & $\sim$ & \url{https://github.com/PCMDI/pmip3-cmor-tables}\\ \hline
			$\sim$ & CORDEX-CMIP6 & & 3.0 & \citet{gutowski_jr_wcrp_2016} & \url{https://github.com/WCRP-CORDEX/cordex-cmip6-cmor-tables}\\ \hline
...
PDRS & PCMDI Data Retrieval and Storage software library (digital format)\\ \hline
